...
type
  ...
  public
    ...
    property OnSelfDestroy : TNotifyEvent read FOnSelfDestroy write FOnSelfDestroy;
  end;
...
constructor TfrToastMessage.Create(const AParent: TWinControl);
begin
  inherited Create(AParent);

  FOnSelfDestroy := nil;
  PositionTop := 30;
  MaxTop := AParent.Height - 80;
  MinTop := AParent.Height;
  StopThread := False;
  ...
  Self.Left := (AParent.ClientWidth - Self.Width) - 50;
...
begin
  StopThread := True;
  if Assigned(FOnSelfDestroy) then
  begin
    FOnSelfDestroy(Self);
  end;
